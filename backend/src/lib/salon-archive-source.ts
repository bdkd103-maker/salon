...
export async function loadSalonArchiveState(tx: Prisma.TransactionClient, salonId: string) {
  ...
  const [bookings, serviceVisits, salonBoosts, barbers, reviews, salonMedia, services, availability, staffMemberships, queueEntries, cards, offers, analyticsEvents, liveStatus, availabilitySubscriptions] = await Promise.all([
    ...
    tx.salonBoost.findMany({ where: { salonId: salonId }, select: { id: true, salonId: true, status: true, durationDays: true, startsAt: true, endsAt: true, amountCents: true, currency: true, providerReference: true, paymentIntentId: true, notes: true, createdAt: true, updatedAt: true } }),
    ...
    tx.offer.findMany({ where: { salonId: salonId }, select: { id: true, salonId: true, title: true, description: true, price: true, isActive: true, availableSlots: true, discount: true, endAt: true, endTime: true, serviceName: true, startAt: true, startTime: true, createdAt: true, updatedAt: true } }),
